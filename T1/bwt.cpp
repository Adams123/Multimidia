...
void encodeBWT(const char *input, const char *output, int block)
{
    ...
    int *indices = (int*)malloc(sizeof(int)*(block+1));
...
void decodeBWT(const char *input, const char *output, int block)
{
    ...
    buffer = (unsigned char *)malloc(sizeof(unsigned char) *(block+1));
    unsigned int *T = (unsigned int *)malloc(sizeof(unsigned int) *(block+1));
    long buflen;
    ...
    for ( ; ; )
    {
        ...
        if ( fread( (char *) &buflen, sizeof(long), 1, stdin ) == 0 )
            break;
        fprintf(stderr,"Processando %ld\n", buflen);
        if ( buflen > ( block + 1 ) )
        {
            fprintf( stderr, "%d Buffer overflow! %ld - %d\n", count, buflen, block+1 );
            abort();
        }
